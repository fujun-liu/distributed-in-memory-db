# -*- coding: utf-8 -*-
"""
Created on Thu Dec  4 17:16:52 2014
...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  4 01:26:10 2014
...
class SimpleHT:
  ...
  def acquire_r_lock(self, key):
      key = key.data
      # lock in use      
      #if key in self.lock:
          #return False
      f = open('/tmp/lock-' + key.replace('/', '') + '.tmp', 'w')
      # go to acquire lock
      try:
...
def main():
  ...
  if "--test" in ol:
    ...
    unittest.main()
    return
  print 'I am runnung on port %d' %port
  serve(port)
...
def serve(port):
  ...
  file_server.register_function(sht.acquire_lock)
  file_server.register_function(sht.release_lock)
